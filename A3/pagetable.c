...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (victim.pte->frame & PG_DIRTY) {
			int swap = swap_pageout(frame, victim.pte->swap_off);
			victim.pte->swap_off = swap;
			evict_dirty_count++;
		} else {
			evict_clean_count++;
		}
 ...
 */
void check_and_handle(pgtbl_entry_t *p, addr_t vaddr) {
	if (p->frame & PG_VALID) { // If the entry is valid 
		hit_count ++;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	p = page_table + p_idx;
	// Check if p is valid or not, on swap or not, and handle appropriately
	check_and_handle(p, vaddr);
	// Make sure that p is marked valid and referenced. Also mark it
	// dirty if the access type indicates that the page will be written to.
    p->frame |= PG_VALID;
    p->frame |= PG_REF;
    if (type == 'M' || type == 'S') { // write or store
        p->frame |= PG_DIRTY;
    }
